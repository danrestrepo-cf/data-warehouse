...
class TestRowGrouper(unittest.TestCase):
    ...
    def test_throws_error_if_one_of_the_given_rows_cannot_be_assigned_a_group(self):
        row_grouper = RowGrouper(MultiKeyMap(['col1']))
    ...
    def test_returns_a_list_of_row_lists_where_the_index_in_the_outer_list_corresponds_to_the_order_of_the_rows_group_number(self):
        row_group_map = MultiKeyMap(key_fields=['col1', 'col2'])
        row_group_map.add_entry({'col1': 1, 'col2': 2}, 0)
        row_group_map.add_entry({'col1': 10, 'col2': 20}, -1)
        row_group_map.add_entry({'col1': 100, 'col2': 200}, 2)
        row_group_map.add_entry({'col1': 1000, 'col2': 2000}, 0)
        row_grouper = RowGrouper(row_group_map)
        test_data = [
