...
class AllStateMachinesGenerator:
    ...
    def __init__(self, data_warehouse_metadata: DataWarehouseMetadata):
        """
        Initialize SingleEtlStateMachineGenerator and GroupStateMachinesGenerator with metadata from EDW's yaml inventory
                ...
                for table in schema.tables:
                    for etl in table.etls:
                        ...
                        etl_state_machine_metadata[etl.process_name] = {
                            ...
                            'comment': ETLMetadata.construct_process_description(table.primary_source_table, table.path, etl.input_type, etl.output_type),
                            'next_processes': sorted(table.next_etls)
                        }
                        # parse DataWarehouseMetadata into structure needed to generate group state machines
    ...
    def create_grouped_config(group_state_machine_metadata: List[dict], group_state_limit: int,
        ...
        for i in range(0, math.ceil(unprocessed_group_state_machine_metadata_length / group_state_limit)):
            if unprocessed_group_state_machine_metadata_length > group_state_limit:
                ...
                parallel_state = StateConfigGenerator.create_root_config(
                    f"{group_state_machine_comment} - group {state_machine_subgroup_suffix}",
                    state_machine_name,
                    {state_machine_name: StateConfigGenerator.create_parallel_config()})
            else:
            ...
            for j in range(0, len(group_state_machine_metadata)):
                ...
                while len(parallel_branches) < group_state_limit and len(group_state_machine_metadata) > 0:
                    ...
                    message_state = StateConfigGenerator.create_root_config(
                        f'Send message to bi-managed-mdi-2-full-check-queue for {process_name}',
                        message_state_name,
                        {message_state_name:
                            StateConfigGenerator.create_message_config(
                                process_name,
                                target_table)})
                    ...
                    message_state_attributes_items.insert(result_selector_position, ('ResultSelector',
                                                                                     StateConfigGenerator.create_result_selector_attribute_config(
                                                                                         message_state_name)))
...
class SingleEtlStateMachineGenerator:
    ...
    def create_step_tree(self, root_process: str) -> dict:
        ...
        root_config = StateConfigGenerator.create_root_config(comment, state_name)
        ...
        if root_process not in self.parent_state_machine_metadata:
            root_config_states[state_name] = StateConfigGenerator.create_task_config(root_process, None, ecs_memory)
            return root_config
        # process has one sequential child
            ...
            root_config_states[state_name] = StateConfigGenerator.create_task_config(root_process, next_process, ecs_memory)
            root_config_states['Load_type_choice'] = StateConfigGenerator.create_choice_config(f'{next_process}_message')
            root_config_states['Success'] = StateConfigGenerator.create_success_config()
            root_config_states[f'{next_process}_message'] = StateConfigGenerator.create_message_config(next_process, next_process_target_table)
            return root_config
        # process has multiple children which should run in parallel
            ...
            root_config_states[state_name] = StateConfigGenerator.create_task_config(root_process, next_processes, ecs_memory)
            root_config_states['Load_type_choice'] = StateConfigGenerator.create_choice_config('Parallel')
            root_config_states['Success'] = StateConfigGenerator.create_success_config()
            root_config_states['Parallel'] = StateConfigGenerator.create_parallel_config()
            for next_process in next_processes:
                ...
                root_config_states['Parallel']['Branches'].append(
                    StateConfigGenerator.create_root_config(
                        f'Send message to bi-managed-mdi-2-full-check-queue for {next_process}',
                        next_process_message_state_name,
                        {next_process_message_state_name: StateConfigGenerator.create_message_config(
                            next_process,
...
class StateConfigGenerator:
    """A collection of functions for creating specific state machine and state machine attribute configuration dicts"""
