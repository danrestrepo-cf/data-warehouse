...
def add_deleted_tables_and_columns_to_history_octane_metadata(metadata: DataWarehouseMetadata,
                                                              history_octane_column_metadata: List[dict]) -> DataWarehouseMetadata:
    ...
    in Octanee. However, EDW's history_octane schema should (and does) contain *deleted* tables/columns that no longer
    exist in Octane but should still be included in the generated metadata. This function essentially cross-references
    the current history_octane information_schema with the metadata pulled from Octane's information_schema in order
    to include previously-deleted tables/columns to the autogenerated metadata resulting from this process.

    :param metadata: a DataWarehouseMetadata object that already contains history_octane metadata within it.
    :param history_octane_column_metadata: a list of dicts with the below structure that contain metadata for all current
