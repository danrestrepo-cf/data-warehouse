...
def restore_non_generated_objects_to_history_octane_metadata(octane_metadata: DataWarehouseMetadata,
                                                             current_yaml_metadata: DataWarehouseMetadata) -> DataWarehouseMetadata:
    """Generate a new DataWarehouseMetadata object with previously-deleted tables/columns and non-physical columns
    incorporated into history_octane by adding data not found in octane_metadata from current_yaml_metadata.
    ...
    currently exist in Octane. However, EDW's history_octane schema contains tables/columns that do not exist in Octane
    but should still be included in the generated metadata.
        ...
        if not combined_history_octane_metadata.contains_table(current_yaml_history_octane_metadata_table.name):
            ...
            for current_yaml_history_octane_metadata_table_column in current_yaml_history_octane_metadata_table.columns:
                ...
                if not combined_history_octane_metadata.get_table(current_yaml_history_octane_metadata_table.name).contains_column(
                    ...
                    if current_yaml_history_octane_metadata_table_column.physical_column_flag is False:
                        # the column is not a physical column so we add it without removing any metadata
                        print(f"        Column is not a physical column... adding to output metadata.")
                        combined_history_octane_metadata.get_table(
                            current_yaml_history_octane_metadata_table.name).add_column(
                            current_yaml_history_octane_metadata_table_column)
                    else:
                        # the column has been deleted from Octane so we add it after removing column metadata
                        print(f"        Column has been deleted... adding to output metadata after removing deleted column data.")
