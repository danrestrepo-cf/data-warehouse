...
def add_deleted_tables_and_columns_to_history_octane_metadata(metadata: DataWarehouseMetadata, # this is the Octane db metadata?
    ...
    in Octane. However, EDW's history_octane schema should (and does) contain *deleted* tables/columns that no longer
    exist in Octane but should still be included in the generated metadata. This function essentially cross-references
    the current history_octane information_schema with the metadata pulled from Octane's information_schema in order  -- information schema or config.mdi?
    to include tables/columns deleted from Octane's database to the autogenerated metadata resulting from this process.

    :param metadata: a DataWarehouseMetadata object that already contains history_octane metadata within it. -- this is built from Octane DB
    :param history_octane_column_metadata: a list of dicts with the below structure that contain metadata for all current -- this is built from config.mdi
    history_octane columns:
        {
            'table_name': <table_name>,
            'column_name': <column_name>,
            'data_type': <data_type>, -- if there is no deleted flag here then there is a (low) possibility we a field with the same name in octane can be deleted, re-created, then deleted again and it would cause an issue
        }
    """
    metadata_copy = copy.deepcopy(metadata)
    try:
        history_octane_schema_copy = metadata_copy.get_database('staging').get_schema('history_octane') # SchemaMetadata
    except InvalidMetadataKeyException:
    ...
    for row in history_octane_column_metadata:
