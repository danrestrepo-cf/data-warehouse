...
pipeline {
    ...
    parameters {
        string(name: 'octane_git_branch', defaultValue: '', description: 'Enter the octane branch id to build. E.g. v2020.1.1.0-release.')
    ...
    environment {
        ...
        CONFIG_METADATA_MAINTENANCE_FILE_OUTPUT_PATH = '**/config_mdi_metadata_maintenance.sql'
    }
    stages {
        stage('Notify') {
            steps {
    ...
    post {
        always {
            archiveArtifacts artifacts: "**/metadata, ${env.CONFIG_METADATA_MAINTENANCE_FILE_OUTPUT_PATH}"
