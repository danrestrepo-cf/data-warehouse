...
set kettle_home_path="C:\Pentaho\Kettle\KETTLE_HOME"

rem Rowlevel, Debug, Error, Basic
set pan_log_level=Debug

call %pan_batch_script_file_location% /DKETTLE_HOME:"%kettle_home_path%" /file:"%transformation_file_location%" /level:%pan_log_level% /param:"input_csv_path_and_filename=%input_csv_path_and_filename%" /param:"database_name=%database_name%" /param:"database_hostname=%database_hostname%" /param:"database_password=%database_password%" /param:"database_port=%database_port%" /param:"database_username=%database_username%"
