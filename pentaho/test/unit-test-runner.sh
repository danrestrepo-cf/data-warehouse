...
function execute_mdi_test_cases() {
  ...
  for dir in ${process_name}/*/; do
    echo "Now executing ${process_name} ${dir}"
    ../../pentaho/test/psql-test.sh ingress ${dir} -f /input/test_case_setup.sql
    execute_mdi_test "$process_name" "$mdi_database_username" "$input_type" "$filename"
    ../../pentaho/test/psql-test.sh ingress ${dir} -f /input/test_case_output.sql
    diff ${dir}/expected_output.csv ${dir}/actual_output.csv || echo ${dir} has differences above
  done
}
...
# execute_mdi_test "SP-0.3"   ${database_username} "none" ""
