...
-- Insert metadata for new tables: deal_note_category_type, doc_status_type
...
SELECT 'Finished inserting metadata for new tables: deal_note_category_type, doc_status_type';

/*
Insert metadata for new columns:
    - proposal_doc.prpd_doc_status_type
...
WITH new_fields (table_name, field_name, field_order) AS (
    VALUES ('proposal_doc', 'prpd_doc_status_type', 55)
        , ('proposal_construction', 'prpc_estimated_permit_amount_applicable', 18)
...
/*
NOTE: proposal_doc table_input_step sql IS OMITTED FROM THE BELOW CTE BECAUSE IT IS UPDATED IN THE SECTION FOR
REMOVING DROPPED COLUMNS. THIS SECTION INCLUDES THE NEW COLUMN BEING ADDED TO proposal_doc.
*/
...
SELECT 'Finished inserting metadata for new columns: proposal_construction.prpc_estimated_permit_amount_applicable,
       asset.as_deposit_date, asset.as_gift_funds_ein, deal_note.dn_category_type, proposal_doc.prp_doc_status_type';
