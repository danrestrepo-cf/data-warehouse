...
SELECT ''deal_pid'' || ''~'' || ''data_source_dwid'' AS data_source_integration_columns
    ...
    , current_deal_stage_records.dst_deal_stage as current_transaction_stage
...
JOIN (
        SELECT <<deal_partial_load_condition>> AS include_record
            , deal_stage.*
            , current_deal_stage_type_records.value as dst_deal_stage
            , etl_log.etl_end_date_time
        FROM history_octane.deal_stage
        ...
        JOIN (        SELECT
                      ...
                      WHERE history_records.code IS NULL) AS current_deal_stage_type_records ON deal_stage.dst_deal_stage_type = current_deal_stage_type_records.code
