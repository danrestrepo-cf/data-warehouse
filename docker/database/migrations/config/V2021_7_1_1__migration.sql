--
-- EDW | star_loan: Add null value handling in calculated fields
-- https://app.asana.com/0/0/1200548658779395
--

/*
Update the field_source_calculation field in the mdi.edw_field_definition table for the listed calculated fields
to evaluate to FALSE against null values in source fields.
*/
UPDATE mdi.edw_field_definition
    SET field_source_calculation = field_source_calculation || ' AND ' || LEFT(field_source_calculation,
        STRPOS(field_source_calculation, ' ') - 1) || ' IS NOT NULL'
    WHERE field_source_calculation IS NOT NULL
        AND field_name IN ('ethnicity_other_hispanic_or_latino_description_flag'
...
/*
Update the field_source_calculation field in the mdi.edw_field_definition table for the listed calculated fields
to evaluate to NULL against null values in source fields.
*/
UPDATE mdi.edw_field_definition
    SET field_source_calculation = 'CASE WHEN primary_table.l_lien_priority_type IS NULL OR t1317.prp_structure_type ' ||
                                   'IS NULL THEN NULL WHEN primary_table.l_lien_priority_type = ''FIRST'' OR ' ||
                                   't1317.prp_structure_type = ''STANDALONE_2ND'' THEN FALSE ELSE TRUE END'
    WHERE field_source_calculation IS NOT NULL
        AND field_source_calculation <> 'primary_table.prp_structure_type = ''COMBO'''
        AND field_name = 'piggyback_flag';


UPDATE mdi.edw_field_definition
    SET field_source_calculation = 'CASE WHEN primary_table.l_lien_priority_type IS NULL OR t1317' ||
                                  '.prp_structure_type IS NULL THEN NULL WHEN primary_table.l_lien_priority_type = ' ||
                                  '''FIRST'' OR t1317.prp_structure_type = ''STANDALONE_2ND'' THEN ' ||
                                  't1317.d_los_loan_id_main ELSE t1317.d_los_loan_id_piggyback END'
    WHERE field_source_calculation IS NOT NULL
        AND field_name = 'los_loan_number';
    ...
    WHERE process_dwid = (
        ...
        FROM mdi.process
            JOIN mdi.table_output_step ON process.dwid = table_output_step.process_dwid
                ...
                AND table_output_step.target_table = 'borrower_demographics_dim'
        );
 ...
 ;'
    WHERE process_dwid = (
        ...
        FROM mdi.process
                 JOIN mdi.table_output_step ON process.dwid = table_output_step.process_dwid
            AND table_output_step.target_schema = 'star_loan'
            AND table_output_step.target_table = 'borrower_lending_profile_dim'
    );
 ...
 ;'
    WHERE process_dwid = (
        ...
        FROM mdi.process
                 JOIN mdi.table_output_step ON process.dwid = table_output_step.process_dwid
            AND table_output_step.target_schema = 'star_loan'
            AND table_output_step.target_table = 'loan_junk_dim'
    );
 ...
 ;'
    WHERE process_dwid = (
        ...
        FROM mdi.process
                 JOIN mdi.table_output_step ON process.dwid = table_output_step.process_dwid
            AND table_output_step.target_schema = 'star_loan'
            AND table_output_step.target_table = 'application_dim'
    );
 ...
 ;'
    WHERE process_dwid = (
        ...
        FROM mdi.process
                 JOIN mdi.table_output_step ON process.dwid = table_output_step.process_dwid
            AND table_output_step.target_schema = 'star_loan'
            AND table_output_step.target_table = 'borrower_dim'
    );
 ...
 ;'
    WHERE process_dwid = (
        ...
        FROM mdi.process
                 JOIN mdi.table_output_step ON process.dwid = table_output_step.process_dwid
            AND table_output_step.target_schema = 'star_loan'
            AND table_output_step.target_table = 'hmda_purchaser_of_loan_dim'
    );
 ...
 ;'
    WHERE process_dwid = (
        ...
        FROM mdi.process
                 JOIN mdi.table_output_step ON process.dwid = table_output_step.process_dwid
            AND table_output_step.target_schema = 'star_loan'
            AND table_output_step.target_table = 'interim_funder_dim'
    );
 ...
 ;'
    WHERE process_dwid = (
        ...
        FROM mdi.process
                 JOIN mdi.table_output_step ON process.dwid = table_output_step.process_dwid
            AND table_output_step.target_schema = 'star_loan'
            AND table_output_step.target_table = 'investor_dim'
    );
...
WHERE process_dwid = (
    ...
    FROM mdi.process
             JOIN mdi.table_output_step ON process.dwid = table_output_step.process_dwid
        AND table_output_step.target_schema = 'star_loan'
        AND table_output_step.target_table = 'lender_user_dim'
);
 ...
 ;'
    WHERE process_dwid = (
        ...
        FROM mdi.process
                 JOIN mdi.table_output_step ON process.dwid = table_output_step.process_dwid
            AND table_output_step.target_schema = 'star_loan'
            AND table_output_step.target_table = 'loan_beneficiary_dim'
    );
 ...
 ;'
    WHERE process_dwid = (
        ...
        FROM mdi.process
                 JOIN mdi.table_output_step ON process.dwid = table_output_step.process_dwid
            AND table_output_step.target_schema = 'star_loan'
            AND table_output_step.target_table = 'loan_funding_dim'
    );
...
UPDATE mdi.table_input_step
    SET sql = 'SELECT NOW( ) AS edw_created_datetime
     ...
     , COALESCE(deal_lender_user.dlu_lender_user_pid, ''<NULL>'') || ''~'' ||
       COALESCE(deal.d_pid, ''<NULL>'') || ''~'' ||
       COALESCE(proposal.prp_pid, ''<NULL>'') || ''~'' ||
       COALESCE(loan.l_pid, ''<NULL>'') || ''~'' ||
 ...
 ;'
    WHERE process_dwid = (
        ...
        FROM mdi.process
                 JOIN mdi.table_output_step ON process.dwid = table_output_step.process_dwid
            AND table_output_step.target_schema = 'star_loan'
            AND table_output_step.target_table = 'mortgage_insurance_dim'
    );
 ...
 ;'
    WHERE process_dwid = (
        ...
        FROM mdi.process
                 JOIN mdi.table_output_step ON process.dwid = table_output_step.process_dwid
            AND table_output_step.target_schema = 'star_loan'
            AND table_output_step.target_table = 'product_choice_dim'
    );
 ...
 ;'
    WHERE process_dwid = (
        ...
        FROM mdi.process
                 JOIN mdi.table_output_step ON process.dwid = table_output_step.process_dwid
            AND table_output_step.target_schema = 'star_loan'
            AND table_output_step.target_table = 'product_dim'
    );
